...
## Decisión
Cada backend tiene un solo lugar central, un manejador global de errores, que convierte cada tipo de error de negocio en un código HTTP, en vez de repetir ese manejo en cada controlador.
...
1. **(SELECCIONADA) Manejo centralizado:** un solo archivo por backend traduce cada tipo de error a su código HTTP.
2. **Manejo local en cada controlador:** cada endpoint captura y traduce sus propios errores por separado.
...
- Es un mecanismo que el framework del proyecto ya trae listo para este problema, sin necesitar ninguna librería extra.
