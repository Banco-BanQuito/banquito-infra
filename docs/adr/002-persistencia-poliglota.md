...
| `routing-service`, `clearinghouse-adapter`, `report-service` | MongoDB | El estado de un pago en tránsito (PaymentBatch, PaymentDetail, CompensationFile) es un documento semi-estructurado de alto volumen y escritura intensiva; no requiere joins relacionales y se beneficia de la escritura flexible de Mongo bajo carga de miles de líneas por segundo. |
